...
with open(sys.path[0]+'/./classification_configuration.json', 'r') as conf:
    ...
    rows_list = []
    # countries = countries[countries['alpha-3'] == 'MWI']
    for i, row in countries.iterrows():
        print(row['alpha-3'])
        try:
            initSheet = pd.read_excel('../data/original/'+row['alpha-3']+'.xlsx', sheet_name='Sanitation Data', usecols='A') 
            
            surveys = [x for x in initSheet.iloc[3:, 0] if str(x) != 'nan' ]
            
            sheet = pd.read_excel('../data/original/'+row['alpha-3']+'.xlsx', sheet_name='Sanitation Data') 
            ...
            for k in [3,4,5]:
                ...
                for idx, column in enumerate(cols):
                    for classification in classifications:
                        if cols[column][classification['row']] != 0 and str(cols[column][classification['row']]) != ".":
                            percent_value =  '%.3f'%(float(str(cols[column][classification['row']]).replace("\xa0", "0.0").replace("ND", "0.0").replace("..","."))*0.01)
                        else: 
                            percent_value = 0
                        if k == 3:
                            context = 'Urban'
            ...
            print(exp)
with open('../data/jmp_sanitation_surveys.csv', 'w', newline='', encoding="utf-8") as file:
